...
echo "downloading latest backup of [postgres] from [${S3_ENDPOINT}/${S3_BUCKET}] ..."
mc ls postgres_backup/${S3_BUCKET}/pgbackup/
mc ls --json postgres_backup/${S3_BUCKET}/pgbackup/ | jq -r -s 'sort_by(.lastModified) | last | .key' | xargs -I{} mc cp postgres_backup/${S3_BUCKET}/pgbackup/{} latest_postgres_backup.sql.gz
